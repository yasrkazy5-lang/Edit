...
function TypingIndicator() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}
         className="msg-ai"
         style={{ padding: 16, maxWidth: 220, display: 'flex', alignItems: 'center', gap: 12,
...
function App() {
  ...
  const sendMessage = async () => {
    ...
    const userMsg = { role: 'user', text, file: attachedFile ? { ...attachedFile } : null, time: now() };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error?.message || 'خطأ في الـ API');
      }

      const data = await res.json();
      const aiText = data.candidates?.[0]?.content?.parts?.[0]?.text || 'ما قدرت أولد رد. جرب مرة ثانية.';
